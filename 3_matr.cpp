...
void otozhdestvlenie(vector <vector<int>>& arr, int& n, int& v1, int& v2) {
	...
	for (int i = 0; i < n; i++) {
		if (log_sum(arr[v1 - 1][i], arr[v2 - 1][i])) { arr[v1 - 1][i] = 1; }
		else { arr[v1 - 1][i] = 0; }
	}

	for (int i = 0; i < n; i++) {
		if (log_sum(arr[i][v1 - 1], arr[i][v2 - 1])) { arr[i][v1 - 1] = 1; }
		else { arr[i][v1 - 1] = 0; }
	}
	...
	case 1:
		...
		case 1:
			...
			if (v1 == v2) { cout << "Невозможно.\n\n"; }
				...
				if (num == 1) { 
					cout << "1 граф до отождествления:\n\n";
					vivod_m(m1, n1);
					otozhdestvlenie(m1, n1, v1, v2); 
					clear(m1, n1);
					cout << "\n";
				}
				else {
					cout << "2 граф до отождествления:\n\n";
					vivod_m(m2, n2);
					otozhdestvlenie(m2, n2, v1, v2); 
					clear(m2, n2);
					cout << "\n";
				}
			}
